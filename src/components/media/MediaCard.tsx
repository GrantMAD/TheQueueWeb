    ...
    <Card isClickable onClick={onClick} className={cn("group relative flex flex-col h-full", className)}>
      <div className="relative aspect-[2/3] w-full overflow-hidden bg-black/5 dark:bg-white/5">
        {item.cover_url ? (
          ...
          <div className="flex h-full w-full items-center justify-center text-gray-400 dark:text-gray-500 bg-black/5 dark:bg-white/5">
            <TypeIcon className="h-12 w-12 opacity-30" />
          </div>
        )}
        
        {/* Gradients and Overlays */}
        <div className="absolute inset-0 bg-gradient-to-t from-white/90 via-white/20 dark:from-black/90 dark:via-black/20 to-transparent opacity-90" />
        ...
        <div className="absolute top-3 right-3">
          <div className="rounded-full bg-white/50 dark:bg-black/50 p-1.5 backdrop-blur-md border border-black/10 dark:border-white/10 text-gray-900 dark:text-white shadow-lg">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
          <h3 className="line-clamp-2 text-sm font-bold text-gray-900 dark:text-white leading-tight drop-shadow-md">
            {item.title}
          </h3>
